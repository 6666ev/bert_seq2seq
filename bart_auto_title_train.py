...
def read_file():
    ...
    with open(zm_tgt_dir, 'r', encoding='utf-8') as f:
        lines = f.readlines()
        for line in lines:
            tgt_zm.append(line.strip('\n').lower())

    with open(xq_tgt_dir, 'r', encoding='utf-8') as f:
        lines = f.readlines()
        for line in lines:
            tgt_xq.append(line.strip('\n').lower())
    
    return src, tgt_zm, tgt_xq
...
class Trainer:
    def __init__(self):
        # 加载数据
        self.sents_src, self.sents_tgt_zm, self.sents_tgt_xq = read_file()
        ...
        # 声明自定义的数据加载器
        dataset = SeqDataset(self.sents_src, self.sents_tgt_zm, self.sents_tgt_xq)

...
class ExtendModel:
    ...
    def my_generate_text(self, text, input_max_length=300, add_eos=False):
        token_out = self.tokenizer.encode(text, max_length=input_max_length)
        if len(token_out) == 2:
            token_ids = token_out[0]
        else:
        ...
        with torch.no_grad():
            ...
            zm_output_ids, xq_output_ids = outputs.logits[0], outputs.logits[1]
    ...
    def sample_generate_encoder_decoder(self, text, input_max_length=300, out_max_length=200, top_k=30, top_p=0.0, add_eos=False):
        ...
        with torch.no_grad():
            for step in range(out_max_length):
                ...
                logit_score = torch.log_softmax(scores[:, -1], dim=-1).squeeze(0) # [21128]
                filtered_logits = top_k_top_p_filtering(logit_score, top_k=top_k, top_p=top_p) # [21128]
                next_token = torch.multinomial(F.softmax(filtered_logits, dim=-1), num_samples=1) #[1]

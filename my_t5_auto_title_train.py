...
src_dir = './data/auto_title_data/train.src'
tgt_dir = './data/auto_title_data/train.tgt'

vocab_path = "state_dict/t5/chinese_t5_pegasus_small/vocab.txt"  # 字典
model_path = "state_dict/t5/chinese_t5_pegasus_small/pytorch_model.bin"  # 预训练参数
...
class Trainer:
    def __init__(self):
        ...
        # self.model.load_all_params(save_model_path)
        # 将模型发送到计算设备(GPU或CPU)
            ...
            if step % 100 == 0:
                ...
                for text in test_data:
                    print(self.model.sample_generate_encoder_decoder(
                        text, add_eos=True, top_k=5))
                self.model.train()
                print("report loss is " + str(report_loss))
                report_loss = 0

            # 因为传入了target标签，因此会计算loss并且返回
            loss = self.model(token_ids, labels=labels_ids,
                              decoder_input_ids=target_ids)[0]
            # 反向传播
            if train:
                # 清空之前的梯度
